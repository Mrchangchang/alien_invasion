...
def check_events(ai_setting, screen, stats,sb, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keyup_event(event, ai_setting, screen, ship, bullets)
...
def fire_bullet(ai_setting, screen, ship, bullets):
    # 创建一颗子弹，并将其加入到编组bullets中
        if len(bullets) < ai_setting.bulltes_allowed:
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    for row_number in range(number_rows):
        for aliien_number in range(number_alien_x):
            # 创建一个外星人并将其加入当前行
            create_alien(ai_settings, screen, aliens, aliien_number, row_number)
...
def create_alien(ai_settings, screen, aliens, aliien_number, row_number):
    # 创建一个外星人并将其加入当前行
    alien = Alien(ai_settings, screen)
    alien_width = alien.rect.width
    alien.x = alien_width + 2 * alien_width * aliien_number
